...
export default function TrainingPage() {
  ...
  const targetEmail = 'user@loveguard.kr';
  ...
  const fetchPersonas = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/training/personas`);
  ...
  const startSession = async (personaId: string) => {
    try {
      ...
      if (data.success) {
        ...
        setMessages([{
          role: 'scammer',
          content: data.data.openingMessage,
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/training/message`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      if (data.success) {
        ...
        setMessages(prev => [...prev, {
          role: 'scammer',
          content: data.data.scammerMessage,
          timestamp: new Date().toISOString(),
        }]);

        const newTurnCount = data.data.turnCount || turnCount + 1;
  ...
  const endSession = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/training/end`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          session_id: session.sessionId,
